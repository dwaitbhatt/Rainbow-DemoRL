...
from rainbow_demorl.utils.replay_buffer_traj import TrajReplayBufferSample, TrajReplayBuffer
from rainbow_demorl.utils.layers import Ensemble, mlp, weight_init

from rainbow_demorl.agents.actors.gaussian_actor_real import GaussianActorReal

class SoftQNetworkReal(nn.Module):
    def __init__(self, obs_space: spaces.Box, act_space: spaces.Box, args: Args):
        '''
        ...
        if args.use_ce_loss:
            self.net = Ensemble([mlp(
                                    np.prod(obs_space.shape) + np.prod(act_space.shape), 
...
class MonteCarloQAgentReal(nn.Module):
    def __init__(self, args: Args, obs_space: spaces.Box, act_space: spaces.Box, device: torch.device):
        super().__init__()
        ...
        ### From ActorCriticAgent
        self.actor = GaussianActorReal(args, obs_space, act_space, device).to(device)

        self.qfs = SoftQNetworkReal(obs_space, act_space, args).to(device)
        self.qfs_target = SoftQNetworkReal(obs_space, act_space, args).to(device)
        ...
        ### From SACAgent
        if args.autotune:
            self.target_entropy = -torch.prod(torch.Tensor(act_space.shape).to(device)).item()
    ...
    def update(self, rb_online: TrajReplayBuffer, rb_offline: TrajReplayBuffer, global_update: int, global_step: int) -> int:
        ...
        for local_update in range(self.args.grad_steps_per_iteration):
            # print(f"[Update] local_update: {local_update}/{self.args.grad_steps_per_iteration}")
            global_update += 1
        ...
        for k, v in target_dict.items():
            if k in source_dict:
                if target_dict[k].shape == source_dict[k].shape:
                    ...
                    raise ValueError(f"[WARN] Shape mismatch on {k}: {source_dict[k].shape} -> {v.shape}")
            else:
