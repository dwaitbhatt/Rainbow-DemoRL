...
class ACTActor(NormalizedActor):
    """
    This is the DETR-like network used in ACT.
    CVAE encoder architecture is a Transformer Encoder, CVAE decoder architecture is a full Encoder-Decoder Transformer
    """
    ...
    def __init__(
        ...
        args: Args,
        backbones: Optional[list[nn.Module]],
    ...
    def forward(self, obs, actions=None, sample_different_actions=False):
        ...
        if is_training:
            ...
            mu = logvar = None
            if sample_different_actions:
                latent_sample = torch.randn([bs, self.latent_dim], dtype=torch.float32).to(state.device)
            else:
        ...
        a_hat = a_hat * self.action_scale + self.action_bias
        return a_hat, [mu, logvar]

    def get_eval_action(self, obs: torch.Tensor) -> torch.Tensor:
        a_hat, (_, _) = self.forward(obs)
        return a_hat

    def to(self, device):
        if self.backbones is not None:
            self.backbones = self.backbones.to(device)
            self.input_proj = self.input_proj.to(device)
    
        self.transformer = self.transformer.to(device)
        self.encoder = self.encoder.to(device)

        self.input_proj_robot_state = self.input_proj_robot_state.to(device)
        self.cls_embed = self.cls_embed.to(device)
        self.encoder_state_proj = self.encoder_state_proj.to(device)
        self.encoder_action_proj = self.encoder_action_proj.to(device)
        self.latent_proj = self.latent_proj.to(device)
        self.latent_out_proj = self.latent_out_proj.to(device)
        self.pos_table = self.pos_table.to(device)
        self.additional_pos_embed = self.additional_pos_embed.to(device)
